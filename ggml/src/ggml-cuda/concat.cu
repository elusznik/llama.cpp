...
void ggml_cuda_op_concat(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const bool is_f32 = src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32;
    const bool is_f16 = src0->type == GGML_TYPE_F16 && src1->type == GGML_TYPE_F16 && dst->type == GGML_TYPE_F16;

    GGML_ASSERT(is_f32 || is_f16);
    ...
    if (ggml_is_contiguous(src0) && ggml_is_contiguous(src1)) {
        if (is_f32) {
            ...
            if (dim != 3) {
                ...
                CUDA_CHECK(cudaMemcpyAsync(dst_d + size0/sizeof(float), src1_d, size1, cudaMemcpyDeviceToDevice, stream));
            }
        } else {
            ...
            if (dim != 3) {
                ...
                CUDA_CHECK(cudaMemcpyAsync(dst_d,                    src0_d, size0, cudaMemcpyDeviceToDevice, stream));
                CUDA_CHECK(cudaMemcpyAsync(dst_d + size0/sizeof(half), src1_d, size1, cudaMemcpyDeviceToDevice, stream));
            }
        }
    } else {
        ...
        if (is_f32) {
            auto launch_kernel = [&](auto dim) {
                concat_non_cont<dim, float><<<grid_dim, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(
            ...
            auto launch_kernel = [&](auto dim) {
                concat_non_cont<dim, half><<<grid_dim, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(
                    (const char *) src0->data, (const char *) src1->data, (char *) dst->data,
                    src0->ne[0], src0->ne[1], src0->ne[2], src0->ne[3],
                    src0->nb[0], src0->nb[1], src0->nb[2], src0->nb[3],
                    src1->ne[0], src1->ne[1], src1->ne[2], src1->ne[3],
